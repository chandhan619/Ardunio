...
const char * service_name = "Test Board";
...
static int light1_Mode = 2;
static int light2_Mode = 1;
static int light3_Mode = 0;
static int light4_Mode = 0;
...
void write_callback(Device * device, Param * param,
  ...
  if (strcmp(device_name, "Light 1") == 0) {
    if (strcmp(param_name, "Power") == 0) {
      ...
      (val.val.b == false) ? digitalWrite(light1, LOW): digitalWrite(light1, HIGH);
      param -> updateAndReport(val);
    }
  }
  else if (strcmp(device_name, "Light 2") == 0) {
    if (strcmp(param_name, "Power") == 0) {
      Serial.printf("Received value = %s for %s - %s\n", val.val.b ? "true" : "false", device_name, param_name);
      (val.val.b == false) ? digitalWrite(light2, LOW): digitalWrite(light2, HIGH);
      param -> updateAndReport(val);
    }
  }
}
...
light1_state = digitalRead(light1_Input);
light2_state = digitalRead(light2_Input);
  ...
  digitalWrite(light2,digitalRead(light2_Input));
  light2_Switch.updateAndReportParam(ESP_RMAKER_DEF_POWER_NAME, light2_state);
...
void setup() {
  ...
  digitalWrite(light2, DEFAULT_POWER_MODE);
  ...
  light2_Switch = Switch("Light 2", & light2);
  light2_Switch.addCb(write_callback);

  my_node.addDevice(light1_Switch);
  my_node.addDevice(light2_Switch);
...
Serial.print("Relay 1 input Switch:");
Serial.println(digitalRead(light1_Input));
//Serial.printf("Relay 2 input Switch: %s\n",digitalRead(light2_Input));
  
  /*----------------------------------------------------------------------------------------------------------------------------------*/  
  //Light 1
  if (light1_Mode == 1) { // Manual Switch
  ...
  } else if (light1_Mode == 2) {  // Touch Sensor
  ...
  /*----------------------------------------------------------------------------------------------------------------------------------*/  
  //Light 2
  if (light2_Mode == 1) { // Manual Switch
    if (light2_state != digitalRead(light2_Input)) { //Push button pressed

      // Key debounce handling
      delay(50);

      if (digitalRead(light2_Input) == LOW) {
        // Toggle device state
        light2_state = LOW;
        Serial.printf("Toggle State to %s.\n", light2_state ? "true" : "false");
        light2_Switch.updateAndReportParam(ESP_RMAKER_DEF_POWER_NAME, light2_state);
        (light2_state == false) ? digitalWrite(light2, LOW): digitalWrite(light2, HIGH);
      } else if (digitalRead(light2_Input) == HIGH) {
        // Toggle device state
        light2_state = HIGH;
        Serial.printf("Toggle State to %s.\n", light2_state ? "true" : "false");
        light2_Switch.updateAndReportParam(ESP_RMAKER_DEF_POWER_NAME, light2_state);
  ...
  } else if (light2_Mode == 2) {  // Touch Sensor

    if (digitalRead(light2_Input) == HIGH) { //Push button pressed

      // Key debounce handling
      delay(50);
      if (digitalRead(light2_Input) == HIGH) {
